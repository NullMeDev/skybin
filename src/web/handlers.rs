...
pub async fn add_comment(
    ...
    if content.len() > 2000 {
        return Err(ApiError(
            "Comment too long (max 2000 characters)".to_string(),
        ));
    }
...
pub async fn admin_stats(
    ...
    let (pastes, comments, sensitive) = db
        .get_db_stats()
        .map_err(|e| ApiError(format!("Database error: {}", e)))?;
...
pub async fn admin_analytics(
    ...
    let scraper_stats = db
        .get_scraper_stats(24)
        .map_err(|e| ApiError(format!("Failed to get scraper stats: {}", e)))?;

    let source_health: Vec<SourceHealthItem> = scraper_stats
        .into_iter()
        .map(|(source, success, failure, pastes)| {
    ...
    let hourly_data = db
        .get_hourly_scrape_rates()
        .map_err(|e| ApiError(format!("Failed to get hourly rates: {}", e)))?;
    let hourly_rates: Vec<HourlyRate> = hourly_data
        .into_iter()
    ...
    let pattern_hits: Vec<PatternHit> = pattern_data
        .into_iter()
        .map(|(pattern, count)| PatternHit { pattern, count })
        .collect();

    // Get source breakdown
    let source_breakdown = db
...
pub async fn admin_activity_logs(
    ...
    let activity_logs: Vec<ActivityLog> = logs
        .into_iter()
        .map(|(action, details, timestamp)| ActivityLog {
            action,
            details,
            timestamp,
        })
...
pub async fn admin_activity_counts(
    ...
    let items: Vec<ActivityCountItem> = counts
        .into_iter()
...
pub async fn admin_bulk_delete(
    ...
    let deleted = db
        .delete_pastes_by_ids(&payload.ids)
...
pub async fn admin_delete_by_search(
    ...
    let deleted = db
        .delete_pastes_by_search(&payload.query)
        .map_err(|e| ApiError(format!("Failed to delete pastes: {}", e)))?;
...
pub async fn forward_telegram_invite(
    ...
    match client
        .post(format!("{}/invite", scraper_url))
        ...
        .await
    {
        ...
        Ok(resp) => Ok(Json(ApiResponse::ok(TelegramInviteResponse {
            forwarded: false,
            message: format!("Scraper returned: {}", resp.status()),
        }))),
        Err(e) => Ok(Json(ApiResponse::ok(TelegramInviteResponse {
            forwarded: false,
