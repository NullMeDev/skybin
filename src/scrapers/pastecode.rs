...
pub struct PastecodeScraper;

impl PastecodeScraper {
    pub fn new() -> Self {
        Self
    }
}

impl Default for PastecodeScraper {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Scraper for PastecodeScraper {
    fn name(&self) -> &str {
        "pastecode"
    }
    ...
    async fn fetch_recent(&self, client: &reqwest::Client) -> ScraperResult<Vec<DiscoveredPaste>> {
        ...
        for cap in re.captures_iter(&html).take(10) {
            if let Some(id) = cap.get(1) {
                ...
                if paste_id.len() >= 4 && paste_id.len() <= 12 {
                    let raw_url = format!("https://pastecode.io/raw/{}", paste_id);
                        ...
                        .await
                    {
                        if let Ok(content) = content_resp.text().await {
                            if !content.is_empty() && content.len() < 100000 {
                                pastes.push(DiscoveredPaste::new("pastecode", paste_id, content)
                                    .with_url(format!("https://pastecode.io/{}", paste_id)));
                            }
                        }
                    }
