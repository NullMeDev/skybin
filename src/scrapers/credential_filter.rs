...
static CREDENTIAL_PATTERNS: Lazy<Vec<Regex>> = Lazy::new(|| {
    vec![
        ...
        Regex::new(r#"(?i)(access[_-]?token|accesstoken)\s*[=:]\s*['"]?[a-zA-Z0-9_-]{16,}"#).unwrap(),
        ...
        // PayPal
        Regex::new(r#"(?i)paypal[_-]?(client[_-]?)?(id|secret)\s*[=:]\s*['"]?[a-zA-Z0-9-]+"#).unwrap(),
        ...
        Regex::new(r#"(?i)(client[_-]?secret|app[_-]?secret)\s*[=:]\s*['"]?[a-zA-Z0-9_-]{16,}"#).unwrap(),
